...
/// 출처·검증센터 페이지.
///
/// 사이트 전체 검증 현황을 요약하고, 모든 출처 목록을 검색 가능한 형태로
/// 제공하며, 오류 제보 메일 작성 버튼을 제공한다.
...
class _SourcesPageState extends State<SourcesPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final sources = repository.sources.where((s) {
      if (_query.isEmpty) return true;
      final q = _query.toLowerCase();
      return s.title.toLowerCase().contains(q) ||
          s.publisher.toLowerCase().contains(q);
    }).toList();
      ...
      child: Column(
        ...
        children: [
          ...
          SearchField(
            ...
            hintText: '출처 제목·발행처 검색',
            onChanged: (value) => setState(() => _query = value),
          ),
          const SizedBox(height: 16),
          if (sources.isEmpty)
            const EmptyStateView(title: '조건에 맞는 출처가 없습니다')
          else
    ...
    return Container(
      ...
      child: Wrap(
        ...
        children: [
          ...
          _stat('검증 대기 항목', '${updates.pendingVerificationCount}개'),
        ],
      ),
    );
  }
